...
const sendRequest = (data, filePath) => {
  const req = http.request(options, res => {
      ...
      res.on('end', () => {
        if (data) {
          ...
          if (
            ...
            console.log(`Import not done, configuration ${filePath} already exists for the ${MEDIATOR_HOSTNAME}`)
            return
          }
          throw Error(`Failed to import configuration ${filePath} for the ${MEDIATOR_HOSTNAME}: ${data.error}`)
        }
      })
    ...
    if (res.statusCode == 201 || res.statusCode == 200 ) {
      ...
      throw Error(`Failed to import configuration ${filePath} for the ${MEDIATOR_HOSTNAME} - ${res.text}`)
    }
  })
...
for (let filePostFix = 0; filePostFix < NUMBER_OF_CONFIG_FILES; filePostFix++) {
  ...
  fs.access(configFilePath, err => {
    if (err) throw Error(`Failed to import the configuration for the ${MEDIATOR_HOSTNAME}: status - ${res.statusCode}`)
